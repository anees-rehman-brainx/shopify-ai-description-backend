...
const productDescription = async (req, res) => {
  try {
    ...
    if (type === 'title') {
      ...
      prompt = `Refine and enhance the following product description. Make it more attractive and fancy based on the provided title. Include bullet points, headings, and reference any similar products available on Amazon if applicable. Return the result in JSON format with the key "refinedDescription".\n\nTitle: ${title}\nDescription: ${description}`;
    } else {
